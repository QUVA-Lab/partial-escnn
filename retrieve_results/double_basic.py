...
def get_runs():
    ...
    # Define the project
    project_name = "double_mnist_FourierFinal"
...
if __name__ == "__main__":
    ...
    # Define filters
    filters = {
        # "dataset_symmetries": "C1",
        # "L_out": 2,
        "rpp": False,
        "restrict": False,
        "learn_eq": False,
        "group": "O2",
        "dataset_symmetries": "O(2)",
        "fourier": True,
        "invariant": False,
    }
    ...
    if args.new_collect_results:
        ...
        # ]
        scores = ["max AugTest Accuracy"]
        to_print = {}
        for key, runs in (grouped_runs).items():
            for score in scores:
                values = []
                for run in runs:
                    values.append(run.history().get(score)[50])
                # print(key, score, f"{np.mean(values):.3f}", f"{np.std(values):.3f}")
                curly_bracket_open = "{"
                curly_bracket_close = "}"
                to_print[
                    key[0]
                ] = f"${np.mean(values):.3f} \ $\myfontsize{curly_bracket_open}$( {np.std(values):.3f})${curly_bracket_close}"
                print(key[0], to_print[key[0]])
        # for data in ["C1", "C4", "SO(2)", "D1", "D4", "O(2)"]:
        #     print(data, to_print[data])

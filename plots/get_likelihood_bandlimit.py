...
network = "O2_on_O2"
bl = 2
dir = f"{network}_bandlimit"
os.makedirs(dir, exist_ok=True)
# fig, host_orig = plt.subplots(ncols=3, nrows=2, figsize=(24, 12), layout="constrained")
fig = plt.figure(figsize=(24, 7))
for i, L in enumerate([1, 2]):
    host = fig.add_subplot(1, 2, i + 1)
    # host = host_orig.flatten()[i]
    ax2 = host.twinx()

    file = pandas.read_csv(f"{dir}/{network}_layer_5_BL_{L}.csv")
    ...
    file = pandas.read_csv(f"{dir}/{network}_layer_5_BL_{L}_error.csv")

    file_2 = pandas.read_csv(f"{dir}/{network}_layer_4_BL_2_error_Full.csv")
...
plt.savefig(f"{dir}/only_1_and_2.pdf", bbox_inches="tight")
# plt.close()

...
dir = "bandlimit_O2_on_SO2"

fig, host_orig = plt.subplots(ncols=3, nrows=1, figsize=(24, 6), layout="constrained")
for i, l in enumerate([0, 2, 3]):
    host = host_orig.flatten()[i]
    ax2 = host.twinx()

    file = pandas.read_csv(f"{dir}/BL_{l}.csv")
    ...
    file = pandas.read_csv(f"{dir}/BL_{l}_error.csv")

    file_2 = pandas.read_csv(f"{dir}/error_Full.csv")
...
plt.savefig(f"{dir}/bandlimit.pdf", bbox_inches="tight")
# plt.close()